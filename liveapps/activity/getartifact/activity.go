...
func (a *Activity) Eval(ctx activity.Context) (done bool, err error) {
	...
	if input.Version != "" {
		queryParams.Set("$version", input.Version)
	}
	...
	if a.settings.FolderType == "Case folder" {
		pathParams["sandbox"] = input.SandboxID
		statusCode, contentType, responseOK, _, _ = contentmanagement.GetCaseFolderArtifact(input.Region, authentication, pathParams, queryParams)
	} else {
		statusCode, contentType, responseOK, _, _ = contentmanagement.GetOrgFolderArtifact(input.Region, authentication, pathParams, queryParams)
	}

	output := &Output{StatusCode: statusCode, ContentType: contentType, Content: responseOK}
