...
export class LiveAppsGetArtifactActivityContribution extends WiServiceHandlerContribution {
    ...
    validate = (fieldName: string, context: IActivityContribution): Observable<IValidationResult> | IValidationResult => {
        ...
        if (fieldName === 'sandboxId') {
            let vResult: IValidationResult = ValidationResult.newValidationResult();
            let folderType: IFieldDefinition = context.getField("folderType");

            if (folderType.value.toLowerCase() === 'case folder'){
                vResult.setVisible(true);
            } else {
                vResult.setVisible(false);
            }
