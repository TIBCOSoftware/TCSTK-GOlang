...
func (a *Activity) Eval(ctx activity.Context) (done bool, err error) {
	...
	if input.Skip != "" {
		queryParams.Set("$filter", input.Skip)
	}
	if input.Top != "" {
	...
	_, response, _, _ := casemanagement.GetTypes(input.Region, authentication, queryParams)

	output := &Output{TypeResponseItems: response}
